...
def ACC(output,target,u=None,region_len=100/3):
    ...
    num_class = 100#int(3*region_len)
...
#     # count number of classes for each region
#     num_class = 100#int(3*region_len)
#     region_idx = torch.cat(((torch.arange(20)/20).long(),torch.ones(80).long()),0)

#     region_vol = [
#     20,80
#     ]
#     target_count = target.bincount().cpu().numpy()
#     region_vol = [target_count[:region_vol[0]].sum(), target_count[region_vol[0]:].sum()]
#     for i in range(len(target)):
#         split_acc[region_idx[target[i].item()]] += correct[i].item()
#     split_acc = [split_acc[i]/region_vol[i] for i in range(2)]

#     print('Classification ACC:')
#     print('\t all \t =',acc)
#     print('\t head \t =',split_acc[0])
#     # print('\t med \t =',split_acc[1])
#     print('\t tail \t =',split_acc[1])
#     return acc, split_acc[0], split_acc[1]
